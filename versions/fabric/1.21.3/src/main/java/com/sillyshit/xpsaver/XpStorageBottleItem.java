...
public class XpStorageBottleItem extends Item {
    ...
    public void appendTooltip(ItemStack stack, Item.TooltipContext context, List<Text> tooltip, TooltipType type) {
        int stored = getStoredXp(stack);
        if (stored > 0) {
            tooltip.add(Text.translatable("tooltip.xp_saver.stored_xp", stored).formatted(Formatting.AQUA));
        } else {
            tooltip.add(Text.translatable("tooltip.xp_saver.empty").formatted(Formatting.GRAY));
        }
    }
